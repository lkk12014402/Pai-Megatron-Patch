...
if [[ "${LLAMA_VER}" = "2.5" ]]; then
    ...
    if [[ "${LLAMA_MODEL_SIZE}" = "7" ]]; then
        ...
        HIDDEN_SIZE=${HL_HIDDEN_SIZE:-3584}
        NUM_HEADS=${HL_NUM_HEADS:-28} # must be divisible by TP
        NUM_QUERY_GROUPS=${HL_NUM_QUERY_GROUPS:-4} # must be divisible by TP
        NUM_LAYERS=${HL_NUM_LAYERS:-28} # must be divisible by PP
        FFN_HIDDEN_SIZE=${HL_FFN_HIDDEN_SIZE:-18944}
        LR=1e-5
        MIN_LR=1e-7

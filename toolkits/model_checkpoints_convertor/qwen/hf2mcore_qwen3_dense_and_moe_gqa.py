...
def convert_checkpoint_from_transformers_to_megatron(hfmodel, mgmodel, args):
    ...
    with torch.no_grad():
        ...
        for mglayer, hflayer in zip(mgmodel.decoder.layers, hfmodel.model.layers):
            ...
            print(qkv_proj.device)
            print(mglayer.self_attention.linear_qkv.weight.device)
            ...
            for pp_rank in range(args.pipeline_model_parallel_size):
                ...
                if pp_rank == 0:
                    for layer in range(5):
                        pp_layer_id = layer
                        layers_to_copy[f"decoder.layers.{pp_layer_id}"] = layer
                elif pp_rank > 0 and pp_rank < args.pipeline_model_parallel_size - 1:
                    for layer in range(9):
                        pp_layer_id = 5 + 9 * (pp_rank - 1)  + layer
                        layers_to_copy[f"decoder.layers.{pp_layer_id}"] = layer
                else:
                    for layer in range(5):
                        pp_layer_id = 5 + 9 * (pp_rank - 1) + layer
                ...
                """
                if pp_rank == 0:
                    specific_layers = [1,2,3,4,5]
                if pp_rank == 1:
                    specific_layers = [6,7,8,9,10,11,12]
                if pp_rank == 2:
                    specific_layers = [13,14,15,16,17,18,19,20]
                if pp_rank == 3:
                    specific_layers = [21,22,23,24,25,26,27,28]
                if pp_rank == 4:
                    specific_layers = [29,30,31,32,33,34,35,36,37]
                if pp_rank == 5:
                    specific_layers = [38,39,40,41,42,43,44,45,46]
                if pp_rank == 6:
                    specific_layers = [47,48,49,50,51,52,53,54,55]
                if pp_rank == 7:
                    specific_layers = [56,57,58,59,60,61,62,63,64]
                for i, layer in enumerate(specific_layers):
                    layers_to_copy[f"decoder.layers.{layer-1}"] = i
                print(layers_to_copy)
                # exit()
                """
